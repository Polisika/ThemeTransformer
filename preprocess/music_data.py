...
class MusicDataset(Dataset):
    def __init__(self, data, max_seq_len, vocab=None, aug=None):
        ...
        self.constants = {
            ...
            "max_tgt_len": min(max_seq_len, max([len(x["tgt"]) for x in self.data]))
            + 1,
        }
    ...
    def data_pitch_augment(self, src_seq, tgt_seq):
        ...
        all_pitches = [self.vocab.getPitch(x) for x in src_seq] + [
            self.vocab.getPitch(x) for x in tgt_seq
        ]
        all_pitches = [x for x in all_pitches if x > 0]
        if len(all_pitches) == 0:
            return src_seq, tgt_seq

        pitch_offsets = np.random.randint(
            1 - min(all_pitches), 127 - max(all_pitches), size=1
        )
    ...
    def __getitem__(self, index):
        ...
        # pitch augment
        src, tgt = self.data_pitch_augment(
            src_seq=self.data[index]["src"], tgt_seq=self.data[index]["tgt"]
        )
