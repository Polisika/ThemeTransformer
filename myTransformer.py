...
class Transformer(Module):
    ...
    def __init__(
        ...
        if custom_decoder is not None:
            ...
            decoder_layer_0 = TransformerDecoderLayer_CrossOnly(
                d_model, nhead, dim_feedforward, dropout, activation
            )
            decoder_layer_1 = TransformerDecoderLayer_parallel(
                d_model, nhead, dim_feedforward, dropout, activation
            )
            decoder_layer_2 = TransformerDecoderLayer(
                d_model, nhead, dim_feedforward, dropout, activation
            )
            ...
            self.decoder = TransformerDecoder_Hybrid(
...
class TransformerDecoder_Hybrid(Module):
    ...
    def __init__(self, decoder_layer, decoder_layer_pattern, xor_pattern, norm=None):
        ...
        self.layers = ModuleList(
            [copy.deepcopy(decoder_layer[x]) for x in decoder_layer_pattern]
        )
...
class TransformerDecoderLayer_CrossOnly(Module):
    ...
    def forward(
        ...
        tgt2, _, self.cross_att_matrix = self.multihead_attn(
            tgt2,
            memory,
            memory,
            attn_mask=memory_mask,
...
class TransformerDecoderLayer_parallel(Module):
    ...
    def forward(
        ...
        tgt2_cross, _ = self.multihead_attn(
            tgt2_cross,
            memory,
            memory,
            attn_mask=memory_mask,
